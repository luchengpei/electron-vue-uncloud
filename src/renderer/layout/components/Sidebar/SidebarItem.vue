    ...
    <div v-if="!item.hidden && item.children" class="menu-wrapper" :class="collapse ? `` : `active-menu-wrapper`">
        <div v-if="item.onlyShowfirst">
            ...
            >
                <el-menu-item :index="resolvePath(onlyOneChild.path)" :class="{ 'submenu-title-noDropdown': !isNest }">
                    <svg-icon
                        v-if="onlyOneChild.meta && onlyOneChild.meta.icon && !onlyOneChild.meta.icon.includes('el')"
                        :icon-class="onlyOneChild.meta.icon"
                    ></svg-icon>
                    <i v-else :class="onlyOneChild.meta.icon"> </i>
                    <span v-if="onlyOneChild.meta && onlyOneChild.meta.title" slot="title"
                        >{{ onlyOneChild.meta.title }}787787</span
                    >
                </el-menu-item>
            </router-link>
        </div>
        ...
        <div v-else>
            ...
            <el-submenu v-else :index="item.name || item.path">
                <template slot="title">
                    <svg-icon
                        v-if="item.meta && item.meta.icon && !item.meta.icon.includes('el')"
                ...
                <template v-for="child in item.children" v-if="!child.hidden">
                    ...
                    <router-link v-else :to="resolvePath(child.path)" :key="child.name">
                        <el-menu-item :index="resolvePath(child.path)">
                            <svg-icon
                                v-if="child.meta && child.meta.icon && !child.meta.icon.includes('el')"
